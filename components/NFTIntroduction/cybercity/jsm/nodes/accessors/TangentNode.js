...
class TangentNode extends Node {
  ...
  generate(builder) {
    ...
    if (scope === TangentNode.GEOMETRY) {
      ...
      outputNode = new VaryingNode(
        new SplitNode(new TangentNode(TangentNode.GEOMETRY), "xyz")
      );
    } else if (scope === TangentNode.VIEW) {
      const vertexNode = new SplitNode(
        new OperatorNode(
          "*",
          new ModelNode(ModelNode.VIEW_MATRIX),
          new TangentNode(TangentNode.LOCAL)
        ),
        "xyz"
      );
      ...
      const vertexNode = new MathNode(
        ...
        new TangentNode(TangentNode.VIEW),
